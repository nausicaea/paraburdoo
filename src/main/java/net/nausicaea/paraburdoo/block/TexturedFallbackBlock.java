...
public abstract class TexturedFallbackBlock extends Block implements PolymerBlock {
    protected TexturedFallbackBlock(Settings settings) {
        super(settings);
    }

    public static TexturedFallbackBlock create(Settings settings, @Nullable Block fallback) {
        if (PolymerBlockResourceUtils.getBlocksLeft(BlockModelType.FULL_BLOCK) > 0) {
            return new Textured(settings);
        } else {
            return new Fallback(settings, fallback);
        }
    }

    private static class Textured extends TexturedFallbackBlock implements PolymerTexturedBlock {
        private final BlockState blockState;

        public Textured(Settings settings) {
            super(settings);
    ...
    private static class Fallback extends TexturedFallbackBlock implements BlockWithElementHolder {
        private final @NotNull Block fallback;

        public Fallback(Settings settings, @Nullable Block fallback) {
            super(settings);
            if (fallback != null) {
